...
void drawText(void) {
	glColor3f(0.2f, 0.0f, 0.4f);
  	glRasterPos2f(7 * WW / 64, 3 * WH / 4);
  	unsigned char string[] = "written by @meganindya for @slowshounakc99";
	glutBitmapString(GLUT_BITMAP_TIMES_ROMAN_24, string);

	if(enableSuccessMessage == false)	goto jump;

	glColor3f(0.1f, 0.4f, 0.1f);
  	glRasterPos2f(11 * WW / 28, WH / 2);
  	unsigned char success[] = "Ball Caught !!";
	glutBitmapString(GLUT_BITMAP_TIMES_ROMAN_24, success);

	jump: if(enableMissMessage == false)	goto skip;

	glColor3f(0.4f, 0.1f, 0.1f);
  	glRasterPos2f(11 * WW / 28, WH / 2);
  	unsigned char missed[] = "Ball Missed !!";
	glutBitmapString(GLUT_BITMAP_TIMES_ROMAN_24, missed);
